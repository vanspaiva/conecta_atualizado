...
function selectOrtog() {
    ...
    switch (ortogSelectValue) {
        ...
        case "Mento":
            ortogImg.setAttribute("src", "https://s3-ap-northeast-1.amazonaws.com/g0v-hackmd-images/uploads/upload_051b1f98dc149d21320324c4d63bdcff.png");
            ortogImg.setAttribute("alt", "Ícone Mento");
...
function changeAtmPesquisa() {
    console.log("chamou");
    item = {
        "idProp": idProp,
        "tipo": "CMF",
        "produto": "ATM",
        "nome": "ATM PESQUISA CLÍNICA",
        "cdg": "ATMPESQUISA",
        "qtd": "1"
    };
    adicionarItemLista(item);
...
function createProductList() {
    ...
    listaTabela.map(function (elem) {
        //console.log(elem.produto);
        if (elem.tipo == "EXTRA") {
            styleDifer = "style='color: red;'";
        }
        if (elem.produto == "SMARTMOLD") {
            document.getElementById("comunidadosmartmold").hidden = false;
            document.getElementById("termosmartmold").setAttribute("required", "");
            $('#btnVideoSmartmold').click();
        }
        const tr = `
                    <tr>
                        <td ${styleDifer}>${elem.tipo}</td>
                        <td>${elem.produto}</td>
                        <td>${elem.nome}</td>
                        <td>${elem.qtd}</td>
    ...
    document.getElementById("msgExtra").hidden = false;
}
...
function apagarLinha(uitem, cdg, qtd) {
    ...
    // Remover qtd de saveQtd
    const qtdIndex = saveQtd.indexOf(qtd);
    if (qtdIndex !== -1) {
        saveQtd.splice(qtdIndex, 1);
    }
...
function setTamanhoRec(op) {
    ...
    // console.log(tamanhoRec);
    // console.log(reconstrucaoOp);
    // console.log(materialRec);

    changeRec(tamanhoRec, reconstrucaoOp, materialRec);
}

function changeRec(tamanho, op, material) {
    console.log(`${tamanho}, ${op}, ${material}`);
