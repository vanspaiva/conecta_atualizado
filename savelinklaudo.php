...
$id = deshashItem($idhashed);

$queryID = mysqli_query($conn, "SELECT * FROM filedownloadlaudo WHERE fileNumPropRef LIKE '%$id%'");
if (mysqli_num_rows($queryID) > 0) {
    while ($row = mysqli_fetch_assoc($queryID)) {
        ...
        header("location: ../laudos?id=" . $id);
    }
} else {
    response(200, "Id não encontrado", $id);
    header("location: ../laudos?id=" . $id . "&error=stmfailed");
}
